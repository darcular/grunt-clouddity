 ...
 */
module.exports.iterateOverClusterNodes = function(options, iterator, done, sync) {
 ...
 *          parameter is passed if an error occurred)
 */
module.exports.iterateOverNodes = function(options, selector, iterator, done, sync) {
  ...
  pkgcloud.compute.createClient(options.pkgcloud.client).getServers({},
      function(err, activeNodes) {
        ...
        if (sync == false){
          async.forEach(data, iterator, done);
        }
        else{
